...
function showDrinkRecipes(drinkRecipes) {
    ...
    for (let i = 0; i < drinkRecipes.length; i++) {
        ...
        // Create and configure the share button
        let shareButton = document.createElement("button");
        shareButton.setAttribute("class", "button is-primary ml-2");
        ...
        nextCardContentDiv.appendChild(saveButton);
        nextCardContentDiv.appendChild(shareButton);
        ...
        shareButton.addEventListener("click", function(event) {
            event.preventDefault();
            const whatsappURL = `https://api.whatsapp.com/send?text=${encodeURIComponent('Check out this cocktail: ' + nextDrinkRecipe.name + ' ' + nextDrinkRecipe.url)}`;
...
function showFavorites(savedDrinks) {
    ...
    for (let i = 0; i < savedDrinks.length; i++) {
        ...
        shareButton.addEventListener("click", function(event) {
            event.preventDefault();
            const whatsappURL = `https://api.whatsapp.com/send?text=${encodeURIComponent('Check out this cocktail: ' + nextRecipe.name + ' ' + nextRecipe.url)}`;
            window.open(whatsappURL, '_blank');
        });
    }
}
